...
void dot_game_exit(void * data)
{
	...
	if(app->upload_scores && !app->demo_file && !app->game.cheats_enabled)
	{
		...
		if(t3net_upload_score(app->leaderboard_submit_url, "dot_to_dot_sweep", DOT_LEADERBOARD_VERSION, "normal", "none", app->user_name, dot_leaderboard_obfuscate_score(app->game.score), buf))
		{
